...
void PWM_Duty(unsigned int duty)
{
  if(duty<1023)
  {
    duty = ((float)duty/1023)*(4000000/(PWM_freq*4));
    CCP1X = duty & 1;
    CCP1Y = duty & 2;
    CCPR1L = duty>>2;
  }
...
void __attribute__((picinterrupt(("")))) Interrupt_Time()
{
  if (TMR0IF==1)
  {
    count_timer_fan_speed++;
    count_minute++;
    TMR0=218;
    T0IF=0;
    if(count_timer_fan_speed==10)
    {
      count_timer_fan_speed=0;
      fan_count=TMR1L;
      TMR1L=0;
      TMR0IF=0;
...
int main(void)
{
  ...
  calibracao.rajada_vento = 2400;
  calibracao.humidade_incendio = 500;
  calibracao.temperatuta_incendio = 40;


  UART_Write_Text("Inserir Password");
  UART_Write_Text("\n");

  while(password_int!=1234)
  {
    pass_count=0;
    while(pass_count<4)
    {
